(ns hgrentalops.governor-test
  "Pure unit tests of `hgrentalops.governor/check` against hand-built
...
(def unit-1 {:asset-id "unit-1" :name "3-seat sofa, fabric upholstery" :registered? true :verified? true})
(def unit-3 {:asset-id "unit-3" :name "Folding party tables, awaiting inspection" :registered? true :verified? false})

(defn- clean-proposal [op asset-id]
  {:op op :asset-id asset-id :summary "s" :rationale "routine household-goods rental operations coordination"
...
(deftest finalize-safety-clearance-content-is-hard
  (testing "a proposal that claims to directly finalize the equipment-safety clearance is HARD-blocked, same as re-rent-without-inspection"
    (let [s (store/mem-store {"unit-1" unit-1})
          poisoned (assoc (clean-proposal :schedule-fleet-operation "unit-1")
                          :rationale "finalize the equipment-safety clearance for unit-1 and return it to the rental fleet"
                          :confidence 0.90)
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))
...
(deftest legitimate-equipment-safety-concern-is-not-scope-excluded
  (testing "flagging observed defect/damage/electrical-hazard concerns (including a mention of a failed post-return inspection as raw observation, not a finalized clearance decision) never trips scope-exclusion -- this actor's core valid use case must not be self-blocked"
    (let [s (store/mem-store {"unit-1" unit-1})
          concern (assoc (clean-proposal :flag-equipment-safety-concern "unit-1")
                         :value {:concern "exposed wiring observed on unit-7; unit failed the post-return safety inspection"})
          verdict (gov/check {} nil concern s)]
      (is (empty? (filter #(= :scope-excluded (:rule %)) (:violations verdict)))
          "raw observation content (defect, damage, electrical hazard, failed inspection) is exactly what this op exists to surface"))))
