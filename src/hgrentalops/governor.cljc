(ns hgrentalops.governor
  "HHGoodsRentalGovernor -- the independent compliance layer that earns
  the HHGoodsRentalOpsAdvisor the right to commit. The advisor has no
  notion of whether a rental-account/equipment record is actually
  ...
  (rental-record logging, fleet-operation/delivery scheduling,
  ...
  (ADR-2607152500): household-goods rental (furniture, appliances,
  party/event equipment) has a modest but real end-user-safety
  dimension -- electrical-appliance defects, party-equipment structural
  issues -- so the closed op allowlist NEVER includes any op that
  directly finalizes an equipment-safety-clearance decision; those are
  always either a hard permanent block or an always-escalate op, never
  auto-commit-eligible. `:flag-equipment-safety-concern` always
  ...
  approval:
    ...
    1. Asset unverified           -- the target rental-account/
                                      equipment record must exist AND
                                      ...
                                      re-derived from the asset's own
  ...
  `hgrentalops.phase` independently agrees:
...
(def high-cost-threshold
  ...
  human, regardless of confidence -- routine consumable/replacement-
  part restocks sit well under this, so this only catches unusually
  large procurement/replacement orders."
  2000)
...
(def scope-excluded-terms
  ...
  a defect, damage report, electrical hazard, or structural issue
  without tripping this gate (see `hgrentalops.governor-test`'s own
  `legitimate-equipment-safety-concern-is-not-scope-excluded` and
  `hgrentalops.advisor-test`'s
  ...
  ["certify safe to re-rent without inspection" "certify this unit safe to re-rent without inspection"
   ...
   "finalize the equipment-safety clearance" "finalize the equipment safety clearance"
...
(defn- high-cost-fleet-restock?
  ...
  `hgrentalops.phase` independently agrees -- `:coordinate-fleet-restock` is no
...
(defn check
  "Censors a HHGoodsRentalOpsAdvisor proposal against the governor
