(ns hgrentalops.store
  "SSoT for the ISIC-7729 renting-and-leasing-of-other-personal-and-
  household-goods OPERATIONS-COORDINATION actor, behind a `Store`
  protocol so the backend is a swap, not a rewrite -- the same seam
  every `cloud-itonami-isic-*` actor in this fleet uses.

  This actor coordinates the back-office operations of a household-
  goods rental fleet (furniture, appliances, party/event equipment):
  rental-record logging (checkout / return / inspection-note data),
  fleet-operation/delivery scheduling, rental-fleet procurement/
  replacement coordination, and equipment-safety-concern flagging
  (defect / damage / electrical hazard / structural issue). It NEVER
  directly finalizes an equipment-safety-clearance decision (e.g.
  certifying a returned unit as safe to re-rent without inspection) --
  see `hgrentalops.governor`'s `scope-exclusion-violations`, a HARD,
  ...
  A registered/verified rental-account/equipment record must exist
  before ANY proposal for that unit may ever commit or escalate --
  `hgrentalops.governor`'s `asset-unverified-violations` re-derives
...
(defprotocol Store
  (asset [s asset-id] "Registered rental-account/equipment record, or
    nil. Unit map: {:asset-id .. :name .. :registered? bool
    :verified? bool}.")
  (all-assets [s])
...
(defn demo-data
  "A small, self-contained household-goods rental fleet directory
  covering both the happy path and the governor's own hard checks, so
  the actor + tests run offline."
  []
  {:units
   {"unit-1" {:asset-id "unit-1" :name "3-seat sofa, fabric upholstery"
              :registered? true :verified? true}
    "unit-2" {:asset-id "unit-2" :name "Full-size refrigerator, 18cu ft"
              :registered? true :verified? true}
    "unit-3" {:asset-id "unit-3" :name "Folding party tables (set of 10), awaiting post-return inspection"
...
(defn seed-db
  "A MemStore seeded with the demo household-goods rental fleet
  directory. The deterministic default."
  []
