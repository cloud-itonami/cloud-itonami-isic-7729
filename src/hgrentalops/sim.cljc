(ns hgrentalops.sim
  ...
  auto-commit), then a fleet-operation scheduling request and a
...
(defn -main [& _]
  (let [db (store/seed-db)
    ...
    (let [r (exec-op actor "t1" {:op :log-rental-record :asset-id "unit-1"
                                  :patch {:renter "Jane Doe" :checkout "2026-07-14" :days 3}} manager-phase-1)]
      (println r)
    ...
    (println (exec-op actor "t2" {:op :log-rental-record :asset-id "unit-1"
                                  :patch {:renter "Jane Doe" :return "2026-07-17"}} manager-phase-3))
    ...
    (println (exec-op actor "t3" {:op :schedule-fleet-operation :asset-id "unit-1"
                                  :patch {:item "delivery re-scheduling" :window "2026-07-20 09:00-12:00"}} manager-phase-3))
    ...
    (println (exec-op actor "t4" {:op :coordinate-fleet-restock :asset-id "unit-1"
                                  :patch {:item "replacement sofa cushions" :quantity 2 :estimated-cost 250}} manager-phase-3))
    ...
    (let [r (exec-op actor "t4b" {:op :coordinate-fleet-restock :asset-id "unit-1"
                                  :patch {:item "replacement sofa unit" :quantity 1 :estimated-cost 3000}} manager-phase-3)]
      (println r)
      (println "-- human fleet manager approves --")
    ...
    (let [r (exec-op actor "t5" {:op :flag-equipment-safety-concern :asset-id "unit-1"
                                 :patch {:concern "frame joint loose, wobbles under load" :confidence 0.92}} manager-phase-3)]
      (println r)
    ...
    (let [actor-direct (op/build db {:advisor (reify advisor/Advisor
      ...
      (println (exec-op actor-direct "t8" {:op :schedule-fleet-operation :asset-id "unit-1"
                                           :patch {:item "annual safety recertification delivery slot"}} manager-phase-3)))
