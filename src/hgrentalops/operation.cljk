(ns hgrentalops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (HHGoodsRentalOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the HHGoodsRentalGovernor (:govern)
  ...
  (:require [langgraph.graph :as g]
            ...
            [hgrentalops.governor :as governor]
            [hgrentalops.phase :as phase]
...
(defn build
  ...
  `hgrentalops.store/Store`).
  opts:
    :advisor      -- a `hgrentalops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; HHGoodsRentalOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; HHGoodsRentalGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
