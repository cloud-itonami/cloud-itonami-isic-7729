(ns hgrentalops.phase
  "Phase 0->3 staged rollout for the ISIC-7729 renting-and-leasing-of-
  other-personal-and-household-goods operations-coordination actor.

    Phase 0  read-only            -- no writes, still governor-gated.
    Phase 1  assisted-logging     -- rental-record logging allowed,
                                      every write needs human approval.
    Phase 2  assisted-coordination-- adds fleet-operation/delivery
                                      scheduling and rental-fleet
                                      restock coordination, still
                                      approval.
  ...
  `hgrentalops.governor`'s own `always-escalate-ops` enforces the
  same invariant independently -- two layers, not one, agree on this."
  (:require [hgrentalops.governor :as governor]))
...
;; auto-commit -- `hgrentalops.governor` independently reaches the same
;; conclusion.
(def phases
...
(defn verdict->disposition
  "Map a HHGoodsRentalGovernor verdict to a base disposition before
  the phase gate."
  [verdict]
